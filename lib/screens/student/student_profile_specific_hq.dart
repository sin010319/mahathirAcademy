...
class SpecificStudentProfileHQ extends StatefulWidget {
  static String id = "/studentProfileSpecificHQ";
  static bool studentDone = false;
  Future studentInfo;
  String studentId;

  SpecificStudentProfileHQ({this.studentId});

  @override
  _SpecificStudentProfileHQState createState() =>
      _SpecificStudentProfileHQState();
}

class _SpecificStudentProfileHQState extends State<SpecificStudentProfileHQ> {
  @override
  void initState() {
    ...
    try {
      Route route = MaterialPageRoute(
          builder: (context) => HQViewFranchiseStudents(
              franchiseId: franchiseId, franchiseName: franchiseName));
  ...
  FutureBuilder<dynamic> closeModal(void value) {
    print('modal closed');
    if (globalCheckEdit == 1 && SpecificStudentProfileHQ.studentDone) {
      Navigator.pop(context);
      Navigator.pop(context);
    } else if (globalCheckEdit == 2 && SpecificStudentProfileHQ.studentDone) {
      Navigator.pushReplacement(context,
          MaterialPageRoute(builder: (BuildContext context) => super.widget));
    }
    SpecificStudentProfileHQ.studentDone = false;
  }
}
